...
reconcile_proxy() {
  ...
  if [ -z "$registry_id" ]; then
    ping_payload="$(jq -cn --arg url "$upstream_url" --arg type "$endpoint_type" \
      '{type:$type,url:$url,credential_type:"basic",access_key:"",access_secret:"",insecure:false}')"
    api_post '/registries/ping' "$ping_payload"

    registry_payload="$(jq -cn --arg name "$endpoint_name" --arg url "$upstream_url" --arg type "$endpoint_type" \
      '{name:$name,url:$url,type:$type,insecure:false,credential:{type:"basic",access_key:"",access_secret:""}}')"
    ...
    actual_type="$(printf '%s' "$registries" | jq -r --arg name "$endpoint_name" \
      '.[] | select(.name == $name) | .type' | head -n 1)"
    if [ "$actual_type" != "$endpoint_type" ]; then
      printf 'endpoint %s has type %s, expected %s\n' \
        "$endpoint_name" "$actual_type" "$endpoint_type" >&2
      exit 1
    fi
...
reconcile_proxy dockerhub dockerhub https://hub.docker.com docker-hub
reconcile_proxy quay quay https://quay.io quay
reconcile_proxy github github https://ghcr.io github-ghcr
reconcile_proxy google google https://gcr.io docker-registry
reconcile_proxy kubernetes k8s https://registry.k8s.io docker-registry
reconcile_proxy elastic elastic https://docker.elastic.co docker-registry
reconcile_proxy nvidia nvidia https://nvcr.io docker-registry
reconcile_proxy ecr-public ecr https://public.ecr.aws docker-registry
